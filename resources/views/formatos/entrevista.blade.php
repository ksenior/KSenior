...
<body>
	<table class="tablaformato" id="boletin bordered">
		<tr>
			...
			<td class="medium "><h4 class="bold">ENTREVISTA A PADRES DE FAMILIA</h4></td>
			<td class="small">
	...
	<table class="bordered contenido mt-2">
		...
		<tr>
			<td class="bold">JORNADA</td><td>{{ $key->jornada }}</td><td class="bold">ACUDIENTE</td><td>{{ $key->acudiente }}</td><td class="bold">PSICOLOGA</td><td>{{ $key->docente }}</td>
		</tr>
		<tr>
			<td class="bold">MOTIVO ENTREVISTA</td><td colspan="5">{{ $key->motivo}}</td>
		</tr>
		<tr>
			<td class="bold" colspan="2">OBSERVACIONES</td>
			<td class="bold" colspan="4">CONCLUSIONES</td>
		</tr>
		<tr>
